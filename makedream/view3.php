...
//댓글 삭제부분
		...
		// 					type: "post",
		// 					data:data_arr,
		// 					url: "../makedream/delete_ripple.php?table=<?=$table?>&num=<?=$item_num?>",
		// 					dataType:"html";
    ...
    //
		...
		// 		        $.ajax({url: "../makedream/write_form3.php?table=<?=$table?>&mode=modify&num=<?=$num?>&page=<?=$page?>", success: function(result){
		// 		            $("#ajax_ripple")
		// 								type: "post",
		// 								data:data_arr,
		// 								url: "../makedream/delete_ripple.php?table=<?=$table?>&num=<?=$item_num?>",
...
//
// 			var req=confirm_action(action, "선택하신 정보를 삭제?");
...
// 수정부분 주혜은추가
// $(document).ready(function(){
// 	$("#insert_ripple_btn").click(function(){
// 		$.get("../makedream/insert_ripple.php?&table=<?=$table?>&num=<?=$item_num?>", function(data.status){
// 			document.write(data);
// 		});
// 	});
// });
...
// $.ajax({
//   url: "../makedream/insert_ripple.php",
//   cache: false
// })
//   .done(function( html ) {
//     $( " #ajax_riripple" ).load( "../makedream/insert_ripple.php" );
//   });
			...
			<hr>
					    ...
					    <ol class="carousel-indicators">
					     ...
					     <div class="carousel-inner" role="listbox">
					       ...
					       <div class="item active">
					         <img src="<?=$img_name ?>" alt="사진" width="460" height="412.5">
					       </div>
								 <?php
				  ...
				  <div class="col-sm-41">
						...
						<div class="pushpush" align="center" id="pushpush">
							<button type="button" id="push_button" class="btn btn-info" onclick="update_chu()">
								<!-- update_push('up','<?=$row['$item_num']?>') -->
							<span class="glyphicon glyphicon-heart" aria-hidden="true"></span> HOPE<br>
							<span id="up_push"><?=$item_up ?></span>
							</button>
						</div>
					</div>
					<hr>
				...
				&nbsp;
					...
					<button type="button" id="delete_button" class="btn btn-default">삭제</button>
				<!-- </a> -->
...
<!-- 리플 출력-->
		<div class="ripple">
			...
			<form  id="ripple_form<?=$item_num?>" name="ripple_form<?=$item_num?>" method="post" action="../makedream/insert_ripple.php?&table=<?=$table?>&num=<?=$item_num?>"
				>
...
<div class="ripple_show">
	<div id="ajax_ripple"></div>
	<div id="show_ripple_div">
			<div id="ripple_writer_title">
				<div class="rrip_title">
					...
					if(($_SESSION['userid']=="admin") || ($_SESSION['userid']==$ripple_id))
								...
								<button type='button' id='delete_ripple_button' class='merong2'>삭제</button></a>";
					?>
				</div>
				</div>
			</div>
			...
			<div class="riripple">
				...
				<!-- 이부분은 원래 숨겨져있어야함 -->
					<div id="hidden_ririple<?=$ripple_num?>" class="collapse">
						<div class="riripple_contentarea" id="show_riripple_div">
							<?php
								...
								$riripple_regist_day=$row_riripple[regist_day];
										...
										?>
													<div class="riripple_wrap" id="riripple_wrap">
														<div id="ajax_riripple"></div>
							 ...
							 ?>
<!-- 리리플 띄우는 곳 -->
					<form  id ="riripple_form<?=$riripple_num?>" name="riripple_form<?=$ripple_num?>" class="riripple_form"  method="post" action="../makedream/insert_ripple.php?&table=<?=$table?>&num=<?=$item_num?>&group=<?=$group_num?>">
						<div class="form-group">
							<!-- <input type=text name=rep> -->
