...
<tr><td>
	<!-- <div class="line1"><div class="line2"> -->
		...
		?>
					...
					<div class="delete_ok">
							<img src="<?=$img_name?>" class="show_saved_pics" width="100px" height="80px">
	...
	if ($mode=="modify" && $item_file[1])
			{
		?>
					<div class="delete_ok"><?php echo $item_file[1]; ?><input type="checkbox" name="del_file[]" value="1"> 삭제</div>
		<?php
			}
		?>
		...
		<?php 	if ($mode=="modify" && $item_file[2])
			{
		?>
					<div class="delete_ok"><?php=$item_file[2]?><input type="checkbox" name="del_file[]" value="2"> 삭제</div>
				</div>
		<?php
			}
			// 확인용
...
<li style="color:#EAEAEA; margin-top: 1px; font-size:13px;">(50MB이하 / PNG,JPG,GIF 형식만 첨부가능)</li>
					<button type="button" value="작성하기" class="write" <?php if($mode=="modify" && (($item_file[0])||($item_file[1])||($item_file[2]))){ $index="1"; } else { $index="2"; } ?>
