<?php
   ...
   if(isset($_POST['user_comm']) && isset($_POST['user_id']))
   {
     ...
     $sql = "insert into reply (parent,depth,id,nick,content,regist_day,ip) values($num,$ripple_depth,'{$_SESSION['userid']}','{$_SESSION['usernick']}','$comment','$regist_day','$ip')";

        mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
        // echo "이거는 댓글이다";

     // 11.26 대댓글 추가 부분
        $sql="select * from reply where parent=$num and id='{$_SESSION['userid']}' and regist_day='$regist_day' and content='$comment';";
