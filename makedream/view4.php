...
// echo "<script>window.alert($_COOKIE[user_count])</script>";
?>
...
$(document).ready(function(){
    // Activate Carousel
    $("#myCarousel<?=$item_num?>").carousel({interval: 3000});
//이부분 총 사진 갯수에 따라 달라지는 초로 계산해야된다 왜냐하면 저건 총 시간(초) 이기때문이다
    // Enable Carousel Indicators
    $(".item1").click(function(){
        $("#myCarousel<?=$item_num?>").carousel(0);
    });
    $(".item2").click(function(){
        $("#myCarousel<?=$item_num?>").carousel(1);
    });
    $(".item3").click(function(){
        $("#myCarousel<?=$item_num?>").carousel(2);
    });

    // Enable Carousel Controls
    $(".left").click(function(){
        $("#myCarousel<?=$item_num?>").carousel("prev");
    });
    $(".right").click(function(){
        $("#myCarousel<?=$item_num?>").carousel("next");
    });
});
...
<body>
	...
	<div id="modal-body">
				<div id="view_content">
					<div class="undersub">
						<span class="undersub_nick">
							<a href="../lib/newindex.php?see=seeuser&user=<?=$item_nick?>">
			...
			<hr>
					    ...
					    <ol class="carousel-indicators">
					     ...
					     <div class="carousel-inner" role="listbox">
					       ...
					       <div class="item active">
					         <img src="<?=$img_name ?>" alt="사진" height="<?=$image_height[0]?>" class="slide_image">
					       </div>
								 <?php
								 ...
								 if($file_count>1)
								 {
									 for($i=1; $i<$file_count; $i++)
									 {
										 ...
										 ?>
										 <div class="item">
							         <img src="<?=$img_name ?>" alt="사진" width="<?=$image_width[$i]?>" height="<?=$image_height[$i]?>" class="slide_image">
							       </div>
									 <?php
									 	}
								 	}
				...
				<div class="idmatch">
					<?php
						if($_SESSION['userid'] && (($_SESSION['userid']==$item_id)) || ($_SESSION['userid']=="Dreamer"))
						{
					?>
					...
					<a href="javascript:del('../makedream/delete.php?table=<?=$table?>&num=<?=$num?>')">
...
<div class="ripple_show">
			...
			<div class="riripple">
				...
				<!-- 이부분은 원래 숨겨져있어야함 -->
					<div id="hidden_ririple<?=$ripple_num?>" class="collapse in">
