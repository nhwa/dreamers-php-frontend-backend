...
// $nick=$_SESSION['usernick'];
  ...
  $place=$_POST['mission_place'];
	$regist_day = date("Y-m-d (H:i)");  // 현재의 '년-월-일-시-분'을 저장
	$hit=0; //조회수 초기화
	// $up=0; //추천수 초기화
	$ip=$_SERVER['REMOTE_ADDR'];  //방문자의 ip 주소 저장
	$mode=$_GET['mode'];
  $num_of_mem=$_POST['sel_people'];
  $deadline=$_POST['mission_date'];
  echo $deadline;

  if($regist_day<$deadline){
    $ingorex=1; //이것의 의미는 진행중이라는 의미이다.
  }
  else{
    $ingorex=0; //이건 종료됐다는 의미임
  }
  ...
  if(($subject=="")||($content==""))
  {
	...
	if ($mode=="modify")
	{
		...
		$sql = "update $table set ingorex=$ingorex, category='$category', subject='$subject', content='$content', place='$place', deadline='$deadline', num_of_mem=$num_of_mem where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
...
$sql="insert into $table (ingorex, id, nick, category, subject, content, place, regist_day,deadline,num_of_mem,file_name_0,file_name_1, file_name_2, file_copied_0, file_copied_1,file_copied_2, hit,ip) ";
$sql.= "values($ingorex,'{$_SESSION['userid']}', '{$_SESSION['usernick']}', '$category', '$subject', '$content', '$place', '$regist_day','$deadline',$num_of_mem,'{$upfile_name[0]}','{$upfile_name[1]}','{$upfile_name[2]}','{$copied_file_name[0]}','{$copied_file_name[1]}','{$copied_file_name[2]}',$hit,'$ip');";
// echo $sql;
...
// echo ("<script>window.alert("안들어감");</script>");
	echo "
	   <script>
	    location.href = '../lib/index-mission.php?page=$page&mode=$mode';
	   </script>
	";
?>
